const mapsRender = () => {
  ...
  function init() {
      ...
      routePanelControl.routePanel.getRouteAsync().then(function (route) {
        ...
        route.model.events.add("requestsuccess", function () {
          ...
          if (activeRoute) {
            ...
            if (!price) {
              showError("Доставка по указанному адресу не осуществляется");
              price = 0;
              classListBtnIcon();
              map.destroy();
              map = null;
    ...
    function setCartDeliveryPrice(price = null) {
      ...
      if ($totalSum !== null) {
        ...
        $totalOldSum.forEach((item) => {
          item.innerHTML = new Intl.NumberFormat("ru-RU").format(totalSum);
        ...
        $deliverySum.forEach((item) => {
          if (item.tagName === "INPUT") {
            ...
            item.innerHTML = new Intl.NumberFormat("ru-RU").format(sum);
          }
        });
      }
    }
    ...
    function calculate(routeLength) {
      if (routeLength > routeMax || routeLength * deliveryTariff > 10000) {
        return false;
      }
    ...
    // function setCartSum(deliverySum){
    //
    //     if($totalSum.length)
    //         document.querySelectorAll('[data-deliverysum]').forEach(item => {
    //             item.innerHTML = new Intl.NumberFormat('ru-RU').format(deliverySum)
    //         })
    //
    //         $totalOldSum.forEach(item => {
    //             item.innerHTML = new Intl.NumberFormat('ru-RU').format(+deliverySum + +$totalSum.innerHtml)
    //         })
    //
    // }

    // const item = $el.closest('[data-modal-target]')
    //
    // item.addEventListener('click', e => {
    //
    //     if (e.target === item || e.target.closest('.close') || e.target.closest('[data-close]')) {
    //
  ...
  //
  // подстановка в input.hidden значения из radio
  //
